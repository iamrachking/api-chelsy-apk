...
<div class="max-w-3xl mx-auto">
    <div class="bg-white rounded-xl shadow-lg border border-gray-200 p-6">
        <div class="mb-6">
            <a href="{{ route('admin.promo-codes.show', $promoCode->id) }}" class="text-blue-600 hover:text-blue-800 mb-4 inline-flex items-center">
                <i class="fas fa-arrow-left mr-2"></i> Retour
            </a>
            <h1 class="text-3xl font-bold text-gray-800 flex items-center mt-4">
                <i class="fas fa-edit text-blue-600 mr-3"></i>
                Modifier le Code Promo : {{ $promoCode->code }}
            </h1>
        </div>
        ...
        <form action="{{ route('admin.promo-codes.update', $promoCode->id) }}" method="POST" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all uppercase" 
                           id="code" type="text" name="code" value="{{ old('code', $promoCode->code) }}" required placeholder="PROMO10">
                    @error('code') 
                ...
                <div>
                    ...
                    <input class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all" 
                           id="name" type="text" name="name" value="{{ old('name', $promoCode->name) }}" required placeholder="Promotion spéciale">
                    @error('name') 
                        <p class="text-red-500 text-xs mt-1 flex items-center">
                            <i class="fas fa-exclamation-circle mr-1"></i>{{ $message }}
                        </p> 
            ...
            <div>
                ...
                <textarea class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all" 
                          id="description" name="description" rows="3" placeholder="Description de la promotion">{{ old('description', $promoCode->description) }}</textarea>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <select class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all" 
                        ...
                        <option value="fixed" {{ old('type', $promoCode->type) === 'fixed' ? 'selected' : '' }}>Montant fixe (FCFA)</option>
                    </select>
                </div>
                ...
                <div>
                    ...
                    <input class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all" 
                           id="value" type="number" step="0.01" name="value" value="{{ old('value', $promoCode->value) }}" required placeholder="10 ou 5000">
            ...
            <div>
                ...
                <input class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all" 
                       id="minimum_order_amount" type="number" step="0.01" name="minimum_order_amount" value="{{ old('minimum_order_amount', $promoCode->minimum_order_amount ?? 0) }}" placeholder="0">
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all" 
                           id="starts_at" type="datetime-local" name="starts_at" value="{{ old('starts_at', $promoCode->starts_at ? $promoCode->starts_at->format('Y-m-d\TH:i') : '') }}">
                ...
                <div>
                    ...
                    <input class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all" 
                           id="expires_at" type="datetime-local" name="expires_at" value="{{ old('expires_at', $promoCode->expires_at->format('Y-m-d\TH:i')) }}" required>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all" 
                           id="max_uses" type="number" name="max_uses" value="{{ old('max_uses', $promoCode->max_uses) }}" placeholder="Illimité si vide">
                </div>
                <div>
                    ...
                    <input class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all" 
                           id="max_uses_per_user" type="number" name="max_uses_per_user" value="{{ old('max_uses_per_user', $promoCode->max_uses_per_user ?? 1) }}" placeholder="1">
                </div>
            </div>

            <div class="flex items-center p-4 bg-gray-50 rounded-lg">
                <input type="checkbox" id="is_active" name="is_active" value="1" {{ old('is_active', $promoCode->is_active) ? 'checked' : '' }} 
            ...
            <div class="flex gap-4 pt-4">
                ...
                <a href="{{ route('admin.promo-codes.show', $promoCode->id) }}" class="flex items-center px-6 py-3 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300 transition-all">
